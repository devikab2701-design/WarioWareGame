...
func Timer(start_time: float): # making a new function for timer countdown!
	...
	while time > 0.0: # run if timer hasnt reached 0
		await get_tree().create_timer(0.1).timeout # asks script to wait on this function. the 'wait' name for the function does nothing here, as await is just telling the scrpit to wait for the function to complete before progressing
		time -= 0.1 # remove 0.1
